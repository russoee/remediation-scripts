<#
.SYNOPSIS
    Disables slide shows on the lock screen to comply with STIG ID WN10-CC-000010.
...
.USAGE
    ...
    ```powershell
    .\WN10-CC-000010_remediation.ps1
    ```
#>
...
if ($Result -eq $RegValue) {
    Write-Host "Slide shows on the lock screen have been disabled successfully." -ForegroundColor Green
} else {
    Write-Host "Failed to disable slide shows on the lock screen." -ForegroundColor Red
}
